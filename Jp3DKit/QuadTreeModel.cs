...
namespace Jp3DKit
{
    public class QuadTreeModel:Model3D
	{
		#region Structures
		public struct Node
		{
        ...
        private HelixToolkit.Wpf.SharpDX.Model3D.EffectTransformVariables effectTransforms;
        ...
        public override void Attach(IRenderHost host)
        {
            try
            {
                ...
                if (terrainModel == null) throw new ArgumentNullException("TerrainMode", "地形模块不能为空");
                if (!SetupTree()) return;
                
                this.TextureRes = ShaderResourceView.FromFile(Device, terrainModel.ModelPath + terrainModel.TextureFileName);
                ...
                this.effectTransforms = new HelixToolkit.Wpf.SharpDX.Model3D.EffectTransformVariables(this.effect);
                ...
                this.effectMaterial.vMaterialAmbientVariable.Set(new Vector4(0.15f, 0.15f, 0.15f, 1.0f));
                this.effectMaterial.vMaterialDiffuseVariable.Set(new Vector4(0.5f, 0.5f, 0.5f, 1f));
                this.effectMaterial.vMaterialSpecularVariable.Set(new Vector4(0.9f,0.9f, 0.9f, 1));
               
                this.Device.ImmediateContext.InputAssembler.InputLayout = this.vertexLayout;
                //vertexBufferBinding = new VertexBufferBinding(this.vertexBuffer, JpSimpleVertex.SizeInBytes, 0);
                /// --- flush
        ...
        public override void Render(RenderContext context)
        {
            ...
            try
            {
                this.effectTransforms.mWorld.SetMatrix(modelMatrix);
                this.effectMaterial.texDiffuseMapVariable.SetResource(this.TextureRes);
                ...
                DrawCount = 0;
                this.frustum.ConstructFrustum(1000,context.ProjectrionMatrix,context.ViewMatrix);

                // Render each node that is visible at the parent node and moving down the tree.
                //Node node=GetLeafNode(ParentNode.Value);
                RenderNode(this.Device.ImmediateContext, ParentNode);
        ...
        {
            if (node.Nodes[0]==null) return node;
            else
				return	GetLeafNode(node.Nodes[0].Value);
        }
		...
		private Node? CreateTreeNode(Device device, float positionX, float positionZ, float width)
		{
			...
			var vertices = new DefaultVertex[vertexCount];
		...
		private void RenderNode(DeviceContext deviceContext, Node? node)
		{
			if (node == null)
				return;
			...
			for (int i = 0; i < 4; i++)
			{
				...
				{
					count++;
					RenderNode(deviceContext, theNode.Nodes[i]);
				}
			}
            ...
            deviceContext.InputAssembler.InputLayout = this.vertexLayout;
            deviceContext.InputAssembler.SetVertexBuffers(0, new VertexBufferBinding(theNode.VertexBuffer, DefaultVertex.SizeInBytes, 0));
			// Set the index buffer to active in the input assembler so it can be rendered.
			deviceContext.InputAssembler.SetIndexBuffer(theNode.IndexBuffer, SharpDX.DXGI.Format.R32_UInt, 0);
            ...
            this.effectPass.Apply(deviceContext);
        ...
        public ShaderResourceView TextureRes { get; set; }
    }
}
